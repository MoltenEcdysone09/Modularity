...
for s in range(1,8):
    for d in range(1,8):
        ...
        if not sdf.empty:
            ccmean = round(np.mean(list(sdf["BC B"])), 3)
...
ax = sns.heatmap(pldf, annot=True, cmap="crest", vmin=0.3, vmax=0.6, cbar_kws={'label': ''})
ax.invert_yaxis()
plt.xlabel("In Degree of B")
plt.ylabel("In Degree of A")
plt.title("BC B")
plt.tight_layout()
plt.savefig("S5C.svg",dpi=300)
plt.savefig("S5C.png",dpi=300, pad_inches=0)
plt.show()
