...
# CC AB with Mean Connectivity
cc = sns.boxplot(ax=axs[0,0],data=mdf, x="Order", y="CC AB", hue="Mean Connectivity")
...
#axs[0,0].set_ylim(-0.9, 0.7)
axs[0,0].legend([],[], frameon=False)
# CC AB with Mean Connectivity
cb = sns.boxplot(ax=axs[0,1],data=mdf, x="Order", y="ConPr", hue="Mean Connectivity")
annotator = Annotator(cb, pairs1, data=mdf, x="Order", y="ConPr", hue="Mean Connectivity")
annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
axs[0,1].legend(title="Fraction of Single High States")
#axs[0,1].set_ylim(-0.9, 0.7)
axs[0,1].legend([],[], frameon=False)
# Fraction of 01 & 10 with Mean Connectivity
cp = sns.boxplot(ax=axs[0,2], data=mdf, x="Order", y="BC A", hue="Mean Connectivity")
annotator = Annotator(cp, pairs1, data=mdf, x="Order", y="BC A", hue="Mean Connectivity")
annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
#axs[0,2].set_ylim(-0.9, 0.7)
axs[0,2].legend(bbox_to_anchor=(1.02, 1), loc=2, borderaxespad=0.)
#axs[0,1].legend([],[], frameon=False)
# BC A with Mean Connectivity
ba = sns.boxplot(ax=axs[1,0], data=mdf, x="Mean Connectivity", y="CC AB", hue="Order")
annotator = Annotator(ba, pairs2, data=mdf, x="Mean Connectivity", y="CC AB", hue="Order")
annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
axs[1,0].legend([],[], frameon=False)
# BC A with Mean Connectivity
bb = sns.boxplot(ax=axs[1,1], data=mdf, x="Mean Connectivity", y="ConPr", hue="Order")
annotator = Annotator(bb, pairs2, data=mdf, x="Mean Connectivity", y="ConPr", hue="Order")
annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
axs[1,1].legend([],[], frameon=False)
# BC A with Mean Connectivity
bc = sns.boxplot(ax=axs[1,2], data=mdf, x="Mean Connectivity", y="BC A", hue="Order")
annotator = Annotator(bc, pairs2, data=mdf, x="Mean Connectivity", y="BC A", hue="Order")
annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
axs[1,2].legend([],[], frameon=False)
#plt.subplots_adjust(left=0.12, right=0.95, bottom=0.12, top=0.95, hspace=0, wspace=0.3)
plt.tight_layout()
plt.savefig("F4.svg",dpi=300)
plt.savefig("F4.png",dpi=300, pad_inches=0)
plt.show()
