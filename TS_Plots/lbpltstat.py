...
cc = sns.boxplot(ax=axs[0,0],data=mdf, x="Mean Connectivity", y="BC B", hue="Order")
...
cp = sns.boxplot(ax=axs[1,0], data=mdf, x="Order", y="BC B", hue="Mean Connectivity")
...
ba = sns.boxplot(ax=axs[0,1], data=mdf, x="Mean Connectivity", y="ConPr", hue="Order")
#annotator = Annotator(ba, pairs, data=mdf, x="Order", y="BC A", hue="Mean Connectivity")
#annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
axs[0,1].legend(title="Order",bbox_to_anchor=(1.02, 1), loc=2, borderaxespad=0.)
axs[0,1].set_ylim(0.2,1.1)
#axs[0,1].legend([],[], frameon=False)


bb = sns.boxplot(ax=axs[1,1], data=mdf, x="Order", y="ConPr", hue="Mean Connectivity")
annotator = Annotator(bb, pairs, data=mdf, x="Order", y="ConPr", hue="Mean Connectivity")
annotator.configure(test='Mann-Whitney', text_format='star').apply_and_annotate()
axs[1,1].legend(title="Mean Connectivity",bbox_to_anchor=(1.02, 1), loc=2, borderaxespad=0.)
axs[1,1].set_ylim(0.2, 1.3)


plt.subplots_adjust(left=0.12, right=0.95, bottom=0.12, top=0.95, hspace=0, wspace=0)
plt.tight_layout()
plt.savefig("S1.svg",dpi=400)
plt.savefig("S1.png",dpi=400, pad_inches=0)
plt.show()
